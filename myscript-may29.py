...
#    all_plots = glob.iglob(pjoin(os.getcwd(),"MATRIX_NNLO_31_inclusive_sumETscale-run/distributions/*.dat"))
    all_plots = glob.iglob(pjoin(os.getcwd(),"datfiles/may29-py8CMS-kQ025-Yon-run/distributions/*.dat"))
...
#        gnu.add_curve(plot_exact,{"format" : "histogram", "label" : "NLO_{PS} (exact)","line_style" : 1})
        gnu.add_curve(plot_scales,{"format" : "histogram", "label" : "MiNNLO_{PS}","line_style" : 2})
...
#            gnu.set_plot_properties("rebin",1)
        ...
        if gnu.get_name().startswith("fid"):
            gnu.set_plot_properties("category","Fiducial")

        if gnu.get_name().startswith("inc-ptH"):
