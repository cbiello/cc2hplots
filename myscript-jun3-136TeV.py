...
#    all_plots = glob.iglob(pjoin(os.getcwd(),"MATRIX_NNLO_31_inclusive_sumETscale-run/distributions/*.dat"))
    all_plots = glob.iglob(pjoin(os.getcwd(),"datfiles/jun3-py8CMS-kQ025-136TeV-run/distributions/*.dat"))
...
#        gnu.add_curve(plot_exact,{"format" : "histogram", "label" : "NLO_{PS} (exact)","line_style" : 1})
        gnu.add_curve(plot_scales,{"format" : "histogram", "label" : "MiNNLO_{PS} (4FS)","line_style" : 2})
...
#            gnu.set_plot_properties("rebin",1)
        ...
        if gnu.get_name().startswith("inc"):
            gnu.set_plot_properties("category","Inclusive")
        if gnu.get_name().startswith("fid"):
            gnu.set_plot_properties("category","Fiducial aa+j_c")
        if gnu.get_name().startswith("fidaa"):
            gnu.set_plot_properties("category","Fiducial aa")

        if "ptH" in gnu.get_name():
            gnu.set_plot_properties("xlabel","p_{T,H}")
            gnu.set_plot_properties("yunit","[fb/GeV]")
            gnu.set_plot_properties("xunit","GeV")
            
        if "yH" in gnu.get_name():
        ...
        if "integrated" in gnu.get_name():
            gnu.set_plot_properties("xlabel","integrated")
            gnu.set_plot_properties("yunit","[fb]")
...
#        gnu.set_plot_properties("process","pp{/Symbol \256}W^+Z{/Symbol \256}e^+ e^{\342\210\222} {/Symbola μ}^+ {/Courier=30 ν}_{/Symbola μ}")
        gnu.set_plot_properties("process","c{/b c\u0305}H")
