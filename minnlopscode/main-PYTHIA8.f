      ...
      logical changescalup,changelepdecay
      real * 8 powheginput,scalupfac
      external powheginput
      integer iun
      common/copencount/iun
      integer py8tune,nohad,noMPI,noQED
      logical alt_recoil
      common/cpy8tune/py8tune,nohad,noMPI,noQED,alt_recoil
      ...
      if(powheginput("#changelepdecay").eq.1) then
         changelepdecay=.true.
      else
         changelepdecay=.false.
      endif
      

      py8tune=powheginput("#py8tune")
      if(py8tune.lt.0) py8tune=14

      nohad=powheginput("#nohad")
      if(nohad.lt.0) nohad=1

      noMPI=powheginput("#nompi")
      if(noMPI.lt.0) noMPI=1

      noQED=powheginput("#noqed")
      if(noQED.lt.0) noQED=1
      
! notice that default is false
      alt_recoil=powheginput("#alt_recoil") == 1
      ...
      do l=1,maxev
         ...
         if(changelepdecay) call py_change_lepdecay
...
c     option as a default.
      ...
      write(*,*) 'not fully tested for ZZJ MiNLO'
